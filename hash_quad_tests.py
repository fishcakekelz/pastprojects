...
class TestList(unittest.TestCase):
    ...
    def test_05(self):
        ...
        ht.insert("dog", [7])
        self.assertEqual(ht.get_num_items(), 2)
        self.assertEqual(ht.get_index("dog"), 2)
        self.assertAlmostEqual(ht.get_load_factor(), 2 / 6)

        ht.insert("mouse", [10])
        self.assertEqual(ht.get_num_items(), 3)
        self.assertEqual(ht.get_index("mouse"), 1)
        self.assertAlmostEqual(ht.get_load_factor(), 3 / 6)

        ht.insert("elephant", [12]) # hash table should be resized
        self.assertEqual(ht.get_num_items(), 4)
        self.assertEqual(ht.get_table_size(), 13)
        self.assertAlmostEqual(ht.get_load_factor(), 4 / 13)
        self.assertEqual(ht.get_index("cat"), 8)
        self.assertEqual(ht.get_index("dog"), 12)
        self.assertEqual(ht.get_index("mouse"), 9)
        self.assertEqual(ht.get_index("elephant"), 1)
        keys = ht.get_all_keys()
        keys.sort()
        self.assertEqual(keys, ["cat", "dog", "elephant", "mouse"])
